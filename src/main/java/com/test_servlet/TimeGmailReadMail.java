...
@Properties({ 
		@Property(name = "sling.servlet.paths", value = { "/timeRead" }),
		})

public class TimeGmailReadMail extends SlingAllMethodsServlet {
	@Reference
...
//          for (int i = arrayMessages.length - 1; i >= 0; i--) {  // this used for print last message only
            for (int i = 0; i < arrayMessages.length; i++) {       // this will print all message
            	...
            	String bodyText="";
            	       bodyText=TimeGmailReadMail.getTextFromMessage1(message);
...
//                   	   out.println("Bodycheckmsg: "+Bodycheckmsg);
            	...
            	if(!GmailMethods.isNullString(subject)){
            		...
            		if(session.getRootNode().hasNode("SentMailTime")){
        				content = session.getRootNode().getNode("SentMailTime");
        			}else{
        				content = session.getRootNode().addNode("SentMailTime");
        			}
            		...
            		if(content.hasNode(subject_replace)){
            			...
            			if(subject_replace_node_mainnode.hasNode("Email_Fields")){
            			 ...
            			 dublicatecheck_mainNode(out, session, subject_replace, Bodycheckmsg, bodyText, toList, from, ccList, sentDate, subject, contentType, message);
            			...
            			if(!subject_replace_node_mainnode.hasNode("Email_Fields")){
						 ...
						 if(!body.hasNode(forid)){
							 ...
							 bodylist.setProperty("SentMailTime", sentDate);
 ...
 public static Node dublicatecheck_mainNode(PrintWriter out, Session session, String subject_replace, String emailBody, String bodyText, String toList, String from, String ccList, 
	  ...
	  Node CheckedNode=null;
	   ...
	   try {
		   ...
		   if(session.getRootNode().hasNode("SentMailTime")){
		   Node readslingnode  = session.getRootNode().getNode("SentMailTime");
			   ...
			   while (iterator.hasNext()) {
				...
				String mainNodeListName=mainNodeList.getName();
					...
					if(mainNodeListName.equals(subject_replace)){
						...
						checkEmailFields_TimeSentMail(out, CheckedNode, emailBody, subject_replace, bodyText, toList, from, ccList, sentDate, subject, contentType,  message);
					}
				}
		   }
...
//							 attachmentlist.setProperty("tomcat_file_link", "http://35.199.31.139:8080/scorpio/"+fileName);
							 ...
							 if(Attachment_With_Type.hasNodes()){
								 ...
								 String file_name=subject_replace+"_"+nodeSize;
								 String subAttachnodename=file_name+"_"+fileName;
								 attachmentlist.setProperty("tomcat_file_link", "http://35.199.31.139:8080/scorpio/"+subAttachnodename);
								 session.save();
							 }
					}
...
public static void checkEmailFields_TimeSentMail(PrintWriter out, Node mainNode, String emailBody, String subject_replace, String bodyText, String toList, String from, String ccList,String sentDate, String subject, 
		 String contentType, Message message){
...
//		out.println("inside :: "+emailBody);
		...
		String slingqery = "select [SentMailTime] from [nt:base] where (contains('SentMailTime','" + sentDate
				+ "')) and ISDESCENDANTNODE('"+path+"')";
...
//		out.println("slingqery:: "+slingqery);
		...
		while (iterator.hasNext()) {
			...
			obj = iterator.nextNode();
			body_Text = obj.getProperty("SentMailTime").getValue().getString();
			
		}
		out.println("SentMailTime: "+body_Text);
		
		out.println("inside data: "+sentDate);

		out.println("length: "+emailBody.length());
		
		
		
		if(body_Text.equals(sentDate)){
